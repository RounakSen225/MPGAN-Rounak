...
class MAB(nn.Module):
    ...
    def forward(self, x: Tensor, y: Tensor, y_mask: Tensor = None, z: Tensor = None):
        ...
        # else:
        x = x + self.attention(x, y, y, attn_mask=y_mask, need_weights=False)[0] # NaN events: tensor([ 50, 110, 117, 128, 191, 208, 231, 416, 439, 488])
        if self.layer_norm:
            x = self.norm1(x)
        x = self.dropout(x)
        x = x + self.ff(x)
...
class GAPT_D(nn.Module):
    ...
    def forward(self, x: Tensor, labels: Tensor = None):
        ...
        tmp = self.final_fc(out)
        return torch.sigmoid(tmp)
